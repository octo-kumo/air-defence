...
import {fromArray2D, type TerrainOptions, toArray2D} from "./core";
...
function convolve(src: TypedArray[], kernel: TypedArray[], tgt?: TypedArray[]): TypedArray[] {
    // src and kernel must be nonzero rectangular number arrays.
    if (!src.length || !kernel.length) return src;
    // Initialize tracking variables.
    let i = 0, // current src x-position
 ...
 */
function gaussianKernel1D(s: number, n = 7) {
    let kernel = new Float64Array(n),
 ...
 */
function gaussian(src: TypedArray[], s: number = 1, kernelSize: number = 7): TypedArray[] {
    if (typeof s === 'undefined') s = 1;
    if (typeof kernelSize === 'undefined') kernelSize = 7;
    let kernel = gaussianKernel1D(s, kernelSize),
 ...
 *   but more accurate smoothing.
 */
export function Gaussian(g: TypedArray, options: TerrainOptions, s = 1, kernelSize = 7) {
