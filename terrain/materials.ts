import {type Material, MeshLambertMaterial, RepeatWrapping, type Shader, Texture} from "three";
import type {Wrapping} from "three/src/constants";
 ...
 *   `MeshLambertMaterial`.
 */
export function generateBlendedMaterial(textures: {
    texture: Texture,
    levels?: number[],
    glsl?: string,
    wrapT?: Wrapping
    ...
    for (let i = 0, l = textures.length; i < l; i++) {
        ...
        declare_ += 'uniform sampler2D texture_' + i + ';\n';
        if (i !== 0) {
            ...
            if (typeof v !== 'undefined') {
                ...
                for (let j = 0; j < v.length; j++) {
                    v[j] = (glslifyNumber(v[j]) as number | string) as number;
                }
            }
            ...
            let blendAmount = !(typeof v !== 'undefined') ? p :
    ...
    let fragBlend = 'float slope = acos(max(min(dot(myNormal, vec3(0.0, 0.0, 1.0)), 1.0), -1.0));\n' +
    ...
    let fragPars = declare_ + '\n' +
        'varying vec2 MyvUv;\n' +
        'varying vec3 vPosition;\n' +
        'varying vec3 myNormal;\n';

    let mat = material || new MeshLambertMaterial();
    mat.onBeforeCompile = function (shader: Shader) {
        // Patch vertexShader to set up MyUv, vPosition, and myNormal
